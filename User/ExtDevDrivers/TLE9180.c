...
#define IFX_TLE9180_SPI_READ_COMMANDS_SIZE (sizeof(IfxTLE9180_readSCArtCommands)/sizeof(IfxTLE9180_SpiTx))
#define IFX_TLE9180_SPI_READ_ERRCOMMANDS_SIZE (sizeof(IfxTLE9180_readErrCommands)/sizeof(IfxTLE9180_SpiTx))
...
const IfxTLE9180_SpiTx IfxTLE9180_readSCArtCommands[]=
{
...
const IfxTLE9180_SpiTx IfxTLE9180_readErrCommands[]=
{
    { .B.C=0, .B.ADDRESS= 0x41, .B.DATA= 0x00, .B.CRC= 2},
    { .B.C=0, .B.ADDRESS= 0x42, .B.DATA= 0x00, .B.CRC= 5},
    { .B.C=0, .B.ADDRESS= 0x43, .B.DATA= 0x00, .B.CRC= 1},
    { .B.C=0, .B.ADDRESS= 0x44, .B.DATA= 0x00, .B.CRC= 0},
    { .B.C=0, .B.ADDRESS= 0x45, .B.DATA= 0x00, .B.CRC= 4},
    { .B.C=0, .B.ADDRESS= 0x46, .B.DATA= 0x00, .B.CRC= 3},
    { .B.C=0, .B.ADDRESS= 0x47, .B.DATA= 0x00, .B.CRC= 7},
    { .B.C=0, .B.ADDRESS= 0x48, .B.DATA= 0x00, .B.CRC= 1},
    { .B.C=0, .B.ADDRESS= 0x49, .B.DATA= 0x00, .B.CRC= 5},
    { .B.C=0, .B.ADDRESS= 0x4A, .B.DATA= 0x00, .B.CRC= 2},
    { .B.C=0, .B.ADDRESS= 0x4B, .B.DATA= 0x00, .B.CRC= 6},
    { .B.C=0, .B.ADDRESS= 0x4C, .B.DATA= 0x00, .B.CRC= 7},
    { .B.C=0, .B.ADDRESS= 0x4D, .B.DATA= 0x00, .B.CRC= 3}
};
...
boolean IfxTLE9180_loadStartupConfiguration(IfxTLE9180 *tle9180)
{
	...
	while(wresult!=TRUE)
	{
		wresult = (tle9180->spiIf.spiExchange(tle9180->spiChannel, (void*)IfxTLE9180_startupConfig, tle9180->receiveBuffer, IFX_TLE9180_SPI_STUP_COMMANDS_SIZE) == 0);
	}
	
	IfxTLE9180_ClrErr(tle9180);
...
boolean IfxTLE9180_readRegister(IfxTLE9180 *tle9180)
{
	...
	status= tle9180->spiIf.spiExchange(tle9180->spiChannel,
			(void*)&IfxTLE9180_readSCArtCommands[tle9180->readIndex],
...
void IfxTLE9180_ClrErr(IfxTLE9180 *tle9180)
{
	volatile uint8 errSize = 0;
	boolean rresult = FALSE;
	for(errSize = 0; errSize < IFX_TLE9180_SPI_READ_ERRCOMMANDS_SIZE; errSize++)
	{
		rresult = FALSE;
		while(rresult!=TRUE)
		{
			rresult = (tle9180->spiIf.spiExchange(tle9180->spiChannel, (void*)&IfxTLE9180_readErrCommands[errSize], &tle9180->receiveBuffer[IFX_TLE9180_SPI_STUP_COMMANDS_SIZE +  IFX_TLE9180_SPI_READ_COMMANDS_SIZE + 2 + errSize], 1) == 0);	
		}
	}
}